...
static void test_ctype_macros(void) {
    ...
    /* Test all values from EOF to 255 (the only range guaranteed by the */
    /* isalpha/isdigit/... contract: representable as unsigned char, or EOF) */
    for (c = -1; c <= 255; c++) {
        /* ft_isalpha */
    ...
    /* NOTE: INT_MAX/INT_MIN removed - passing them to the original glibc */
    /* isalpha/isdigit/etc. is undefined behavior (argument must be */
    /* representable as unsigned char or be EOF) and segfaults on glibc's */
    /* table-lookup based implementation. This is a limitation of testing */
    /* against the real function, not a bug in ft_* implementations. */
    int extra_vals[] = {300, -300, EOF};
...
static void test_tolower_toupper(void) {
    ...
    int c;
    /* Range restricted to EOF..255 - the only domain guaranteed by the */
    /* tolower/toupper contract. Values below -1 are undefined behavior: */
    /* glibc happens to normalize them internally (c+256) due to its table */
    /* layout, which a spec-compliant ft_tolower/ft_toupper is not required */
    /* to replicate. */
    for (c = -1; c <= 255; c++) {
        int exp = tolower(c);
